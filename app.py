    ...
    if request.method == 'POST':
        try:
            ...
            if not firstname or not lastname:
                ...
                cursor.execute(
                    'INSERT INTO employeeInfo (firstName, lastName) VALUES (?, ?)', (firstname, lastname))
...
def login():
    ...
    try:
        ...
        cursor.execute(
            'SELECT EXISTS (SELECT * FROM employeeInfo WHERE employeeId=?)', (employeeId))
...
def startTime(employeeId):
    ...
    try:
        ...
        if len(temp) > 0:
            update = False
            message = "Can't Start Shift Becasue of Active Shift"
        if update:
